...
// `du` does, instead of once per link. Windows drops inode-based dedup (no
// portable inode surface through fs.FileInfo), so the semantics are macOS-only.
func TestScanPathConcurrentDedupsHardlinks(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("hardlink dedup is a macOS du-compatibility behavior")
	}
...
func TestPruneAnalyzerCacheDirIgnoresRemoveFailures(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		// Windows has no POSIX mode bits and os.Remove transparently clears
		// the read-only attribute, so a failed removal cannot be arranged.
		t.Skip("cannot simulate remove failure on Windows")
	}
...
// getCacheDir memoizes MkdirAll, so it has to notice when the cache root moves
// (LOCALAPPDATA on Windows, HOME-derived elsewhere) or every test after the
// first would write into the first one's temp directory.
func TestGetCacheDirFollowsHomeChanges(t *testing.T) {
	firstHome := t.TempDir()
	t.Setenv("HOME", firstHome); t.Setenv("USERPROFILE", firstHome)
	if os.Getenv("LOCALAPPDATA") != "" {
		t.Setenv("LOCALAPPDATA", filepath.Join(firstHome, "AppData", "Local"))
	}
	...
	t.Setenv("HOME", secondHome); t.Setenv("USERPROFILE", secondHome)
	if os.Getenv("LOCALAPPDATA") != "" {
		t.Setenv("LOCALAPPDATA", filepath.Join(secondHome, "AppData", "Local"))
	}
...
func TestScanPathPermissionError(t *testing.T) {
	// Windows has no POSIX mode bits; chmod 000 is a no-op for directory
	// traversal, so the locked-directory scenario does not exist there.
	if runtime.GOOS == "windows" {
		t.Skip("no POSIX permission model on Windows")
	}
...
func TestSystemOverviewRootsDefaultsToRealSystemPaths(t *testing.T) {
	...
	if len(roots) < 1 {
		t.Fatalf("expected at least one system root, got %d", len(roots))
	}
	for _, root := range roots {
		// Every root is a drive letter followed by :\ (e.g. C:\)
		if len(root.Path) != 3 || root.Path[1] != ':' || root.Path[2] != '\\' {
			t.Fatalf("unexpected system root %q, want a drive root like C:\\", root.Path)
		}
...
func TestResolveScanTargetRouting(t *testing.T) {
	cases := []struct {
		...
		{name: "no target is the overview scan", wantOverview: true, wantPath: filepath.VolumeName(os.Getenv("SystemRoot")) + `\`},
		{name: "explicit arg is a directory scan", args: []string{`C:\tmp`}, wantPath: `C:\tmp`},
		{name: "env target is a directory scan", envPath: `C:\tmp`, wantPath: `C:\tmp`},
		{name: "env target wins over args", envPath: `C:\tmp`, args: []string{`D:\var`}, wantPath: `C:\tmp`},
