...
func getDirectorySizeFromDuWithExcludeAndIgnores(path string, excludePath string, ignoreNames []string) (int64, error) {
	return 0, fmt.Errorf("du is not available on Windows")
}
...
// Windows has no portable inode-based dedup surface through fs.FileInfo, so
// hardlinked files are counted once per visible name (NTFS deduplicates on
// disk anyway). The bool always reports false.
