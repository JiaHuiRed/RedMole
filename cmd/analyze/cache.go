...
func moleCacheRoot(home string) string {
	// Windows keeps per-user app state under %LOCALAPPDATA% rather than a
	// hidden dot-directory in the profile root.
	if appData := os.Getenv("LOCALAPPDATA"); appData != "" {
		return filepath.Join(appData, "mole")
	}
...
// and LOCALAPPDATA values it was derived from, so a test that repoints either
// still gets a fresh answer.
...
func getCacheDir() (string, error) {
	...
	if resolved := cachedAnalyzerDir.Load(); resolved != nil && resolved.home == home && resolved.appData == appData {
		return resolved.dir, resolved.err
	}

	dir := filepath.Join(moleCacheRoot(home), analyzerCacheDirName)
	resolved := &resolvedCacheDir{home: home, appData: appData, dir: dir}
...
func loadRawCacheFromDisk(path string) (*cacheEntry, error) {
	...
	if err := decoder.Decode(&entry); err != nil {
		...
		// costs a block until some prune pass reaches it. Close before remove:
