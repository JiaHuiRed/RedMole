...
- **`--depth N` 参数** — JSON 模式支持目录深度控制，一次扫描拿到嵌套子目录，不用反复下钻
- **`--top N` 参数** — 快速返回最大的 N 个条目，适合"先看最大头在哪"的场景
- **非交互终端自动 JSON** — stdout 被 pipe/重定向时自动切 JSON，不用手动加 `-json`
- **`size_human` 字段** — JSON 输出附带人类可读大小，减少下游换算

### 🐛 修复

- **JSON 下钻盲区** — 之前 `-json` 只返回顶层，深层目录需要反复手动扫描；现在通过 `--depth` 一次性解决
